...
-->
# Introduction
  The world of Pokémon began in 1995 with the pair of games Pokémon Red and Green. For non-Japanese
audiences, the latter would become known as Pokémon Blue. These two games introduced the turn-based
battling system of Pokémon. While numerous other games have copied the Pokémon battling format, none have been able to equal its widespread appeal and dedicated player base. With each successive iteration items, Pokémon types, and of course new Pokémon are added to the Pokémon lexicon. The world of Pokémon has continued to grow and evolve into one of the largest video games franchises. The most recent iterations of Pokémon games, Pokémon Omega Ruby and Alpha Sapphire, have continued the tradition of adding layers to an already complex system of battling. Recently with the introduction of Pokémon Sun and Moon, the Pokémon battling formula is once again being revitalized with additional intricacies. 

  There are a host of games and programs unaffiliated with the Pokémon franchise that have allowed players to test and practice their Pokémon battling skills. The most widely used of such programs is Pokémon Showdown. Since 2011 the program Pokémon Showdown has offered a 'stripped' version of Pokémon games. This 'stripped' version uses only the Pokémon battling system, replicating the most recent iteration of the Pokémon battling system in the process. This has allowed players to hone and test their Pokémon battling skills. With well-over 12,000 daily registered users and counting, this program has become the go-to program to test and practice battling strategies in the ultimate pursuit of becoming the very best that no one ever was.

To begin with however, Pokémon battling must be explicitly detailed for both laymen and game theorists alike. 

### A Brief Overview 
First, there needs to be a formal model. At its core, Pokémon battling takes place exclusively between two players. Each of the two players has a team composed of six different Pokémon. Depending on the battle format, the composition of the six Pokémon is either dictated by the player or randomly assigned. Regardless of format, each turn each player simmultaneously makes a decision. The decisions are then executed, with priority given first to priority moves and then by a comparative assessment of speed. Once a player's Pokémon loses its health, the Pokémon can no longer be used. This Pokémon has fainted, forcing the player whose Pokémon fainted to switch into another Pokémon. After all of the Pokémon on one team have fainted, or one player forfeits, the battle ends. As would be expected, the player whose Pokémon have not fainted win the battle. The ways to end a battle are by forfeiture or by having all of their Pokémon faint before their opponents; the latter outcome is considered a "normal" game. 

There are a number of parameters further detail. Each Pokémon has four moves that it can execute. However, whenever a player has at least two Pokémon that have not fainted they are able to switch. Thus, players almost always have five different choices to make. However if each potential Pokémon to switche into is considered unique, players have at most nine distinct potential decisions per turn. There are a number of other parameters to consider. Such parameters will be further detailed in the methodology section, along with greater details on specific move types. As each game is composed of a sequence of turns, Pokémon battling is a sequential game. Overall, Pokémon battling is a sequential zero-sum game with perfect recall and imperfect information. Additionally, each player is able to see all past decisions. This makes Pokémon battling a perfect recall game. 

There are only two outcomes to any game: Win or Lose. Due to this, Pokémon is a zero-sum game. Nonetheless there are distinct ways that a player may win or lose a game. Notably, the same set of decisions may not lead to the same outcome, i.e. for  $i \neq j$ if $X_{1,i,T} = Win$ and $X_{1,i,T} \equiv X_{1,j,T}$ then $\neg(X_{1,j,T} \rightarrow Win)$. That is to say, there is independence between the indexing of battles. This highlights a unique trait of Pokémon battling: the role of information. 

Let the decisions of players 1 and 2 in their ith game be denoted respectively by $X_{1,i,t}, X_{2,i,t}$. Let the final turn be denoted n and the set of all turns T. It follows then that the set of all decisions made by player 1 in their i-th Pokémon battle is denoted $X_{1,i,T} = ({X_{1,,i,1}, X_{1,i,2},...,X_{1,i,n}})$ Due to the construction of the game, and specific note of general format restrictions, n is finite. 

That being said, my work expands upon previous literature in game theory and behvaioral economics. The analysis of Pokémon does not apply soley to the game itself. By connecting Pokémon battling into the wide array of game theory literature, I hope to bridge computationally intensive and qualitative research into a syncretic mix that not only justifies the rigorous analysis of Pokémon, but also motivates future research. To do so, a number of concepts from game theory and data analysis need to be considered.  
