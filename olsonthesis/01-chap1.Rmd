...
There are a number of parameters further detail. Each Pokémon has four moves that it can execute. However, whenever a player has at least two Pokémon that are not fainted, they are able to switch. Thus, players almost always have five different choices to make, though if switches are considered specifically the player may actually have nine choices to choose for a given turn. There are a number of other parameters to consider. Such parameters will be further detailed in the methodology section, along with greater details on specific move types. 
