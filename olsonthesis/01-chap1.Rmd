...
battling system of Pokémon. Numerous other games have copied the Pokémon battling format, though none have been able to equal its widespread appeal and dedicated player base. With each successive iteration, new items, types, and of course Pokémon are added to the Pokémon lexicon. The world of Pokémon has continued to grow and evolve into one of the largest video games franchises. The most recent iterations of Pokémon games, Pokémon Omega Ruby and Alpha Sapphire, have continued the tradition of adding layers to an already complex system of battling. 
...
### A Brief Overview 
First, there needs to be a formal model. At its core, Pokémon battling takes place exclusively between two players. Each of the two players has a team composed of six different Pokémon. Depending on the battle format, the team of six Pokémon is either dictated by the player or randomly assigned. Regardless of format, each player simmultaneously makes a decision. The decisions are then executed, with priority given first to priority moves and by a comparative assessment of speed. Once a player's Pokémon loses its health, called fainting, the player must switch into another Pokémon. After all of the Pokémon on one team have fainted, the battle ends. As would be expected, the player whose Pokémon have not fainted win the battle. 

There are a number of parameters and details to note further. Each Pokémon has four moves that it can execute. However, whenever a player has at least two Pokémon that are not fainted, they are able to switch. Thus, players almost always have five different choices to make, though if switches are considered specifically the player may actually have nine choices to choose for a given turn. There are a number of other parameters to consider. Such parameters will be further detailed in the methodology section, along with greater details on specific move types. 
